...
booking_data_file_path = "food_orders_data.csv"
...
def validate_response(payload: dict):
    ...
    print(payload)
    try:
        booking_details['orderId'] = payload['orderId']
        booking_details['customerId'] = payload['customerId']
        booking_details['dishType'] = payload['dishType']
        booking_details['orderDate'] = datetime.strptime(payload['orderDate'], '%d-%m-%Y')
        booking_details['price'] = float(payload['price'])
